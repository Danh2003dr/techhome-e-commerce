          ...
          <nav className="container mx-auto px-6 py-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <div className="relative hidden lg:block">
                <span className="absolute inset-y-0 left-3 flex items-center text-slate-400"><span className="material-icons text-sm">search</span></span>
                <input className="pl-10 pr-4 py-2 bg-slate-100 dark:bg-slate-800 border-none rounded-full text-sm focus:ring-2 focus:ring-primary w-64" placeholder="Search products..." type="text" />
              </div>
              <button type="button" className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full"><span className="material-icons">favorite_border</span></button>
              <Link to="/cart" className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full relative">
                <span className="material-icons">shopping_cart</span>
                {cartCount > 0 && (
                  <span className="absolute top-1 right-1 bg-primary text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center font-bold border-2 border-white dark:border-background-dark">{cartCount > 99 ? '99+' : cartCount}</span>
                )}
              </Link>
            </div>
          </nav>
                ...
                {extras!.relatedProducts.map((item) => (
                  <div key={item.id} className="group">
                    <div className="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-200 dark:border-slate-800 mb-4 overflow-hidden relative">
                      ...
                      <button type="button" className="absolute top-4 right-4 bg-white/80 dark:bg-black/40 backdrop-blur p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"><span className="material-icons text-sm">favorite_border</span></button>
                    </div>
        ...
        <footer className="bg-white dark:bg-background-dark border-t border-slate-200 dark:border-slate-800 py-12">
          <div className="container mx-auto px-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-12">
              <div><h5 className="font-bold mb-4">Shop</h5><ul className="space-y-2 text-sm text-slate-500"><li><Link to="/search" className="hover:text-primary">iPhone</Link></li><li><a href="#" className="hover:text-primary">MacBook</a></li><li><a href="#" className="hover:text-primary">iPad</a></li><li><a href="#" className="hover:text-primary">Apple Watch</a></li></ul></div>
              <div><h5 className="font-bold mb-4">Support</h5><ul className="space-y-2 text-sm text-slate-500"><li><a href="#" className="hover:text-primary">Order Status</a></li><li><a href="#" className="hover:text-primary">Returns</a></li><li><a href="#" className="hover:text-primary">Contact Us</a></li><li><a href="#" className="hover:text-primary">FAQ</a></li></ul></div>
              <div><h5 className="font-bold mb-4">Company</h5><ul className="space-y-2 text-sm text-slate-500"><li><a href="#" className="hover:text-primary">About TechHome</a></li><li><a href="#" className="hover:text-primary">Careers</a></li><li><a href="#" className="hover:text-primary">Sustainability</a></li><li><a href="#" className="hover:text-primary">Newsroom</a></li></ul></div>
              <div><h5 className="font-bold mb-4">Newsletter</h5><p className="text-sm text-slate-500 mb-4">Get the latest tech news and exclusive offers.</p><div className="flex"><input className="flex-grow bg-slate-100 dark:bg-slate-800 border-none rounded-l-lg text-sm px-4 py-2 focus:ring-1 focus:ring-primary" placeholder="Email address" type="email" /><button type="button" className="bg-primary text-white px-4 py-2 rounded-r-lg text-sm font-bold">Join</button></div></div>
            </div>
            <div className="border-t border-slate-100 dark:border-slate-800 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
              <p className="text-xs text-slate-400">© 2024 TechHome Inc. All rights reserved.</p>
              <div className="flex gap-6 text-xs text-slate-400"><a href="#" className="hover:text-primary">Privacy Policy</a><a href="#" className="hover:text-primary">Terms of Service</a><a href="#" className="hover:text-primary">Cookies Settings</a></div>
              <div className="flex gap-4"><span className="material-icons text-slate-400 cursor-pointer hover:text-primary transition-colors">facebook</span><span className="material-icons text-slate-400 cursor-pointer hover:text-primary transition-colors">camera_alt</span><span className="material-icons text-slate-400 cursor-pointer hover:text-primary transition-colors">alternate_email</span></div>
            </div>
          </div>
        </footer>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-20">
        <div className="lg:col-span-7 flex flex-col-reverse md:flex-row gap-4">
          <div className="flex md:flex-col gap-3 overflow-x-auto md:overflow-visible">
            {images.map((img, num) => (
              <button key={num} type="button" onClick={() => setSelectedImageIndex(num)} className={`w-20 h-20 border rounded-xl overflow-hidden transition-all flex-shrink-0 ${selectedImageIndex === num ? 'border-indigo-600' : 'border-gray-200 hover:border-indigo-600'}`}><img src={img} alt={`Thumb ${num + 1}`} className="w-full h-full object-cover" /></button>
            ))}
          </div>
          <div className="flex-grow bg-white border border-gray-100 rounded-3xl overflow-hidden flex items-center justify-center p-8 group relative">
            ...
            <button type="button" className="absolute bottom-6 right-6 bg-white/80 p-2.5 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-opacity"><span className="material-icons">zoom_in</span></button>
          </div>
        </div>
        <div className="lg:col-span-5">
          <div className="mb-6">
            ...
            <div className="flex items-center gap-4">
              <div className="flex text-yellow-400">{[...Array(5)].map((_, i) => <span key={i} className={`material-icons text-sm ${i < Math.floor(product.rating) ? 'text-yellow-400' : 'text-gray-200'}`}>star</span>)}</div>
          ...
          <div className="mb-8">
            <div className="flex items-baseline gap-3"><span className="text-3xl font-black text-gray-900">${product.price.toFixed(2)}</span>{product.oldPrice && <span className="text-lg text-gray-400 line-through">${product.oldPrice.toFixed(2)}</span>}</div>
            <p className={`text-sm font-bold mt-1 ${product.inStock !== false ? 'text-emerald-600' : 'text-red-600'}`}>{product.inStock !== false ? 'In Stock - Ready to ship' : 'Out of Stock'}</p>
          </div>
          <div className="space-y-8">
            {colors.length > 0 && (
              <div>
                ...
                <div className="flex gap-3">{colors.map((color, idx) => <button key={idx} type="button" onClick={() => setSelectedColor(color.name)} className={`w-10 h-10 rounded-full border-2 transition-all ${selectedColor === color.name ? 'border-indigo-600 p-0.5' : 'border-transparent'}`}><div className="w-full h-full rounded-full" style={{ backgroundColor: color.hex }} /></button>)}</div>
              </div>
            )}
            {storageOptions.length > 0 && (
              <div>
                <label className="block text-sm font-bold text-gray-900 mb-3">Storage Capacity</label>
                <div className="grid grid-cols-4 gap-3">{storageOptions.map((size) => <button key={size} type="button" onClick={() => setSelectedSize(size)} className={`py-3 rounded-xl border text-sm font-bold transition-all ${selectedSize === size ? 'bg-indigo-50 border-indigo-600 text-indigo-600' : 'border-gray-200 text-gray-500 hover:border-indigo-600'}`}>{size}</button>)}</div>
              </div>
            )}
            <div className="flex gap-4">
              <button disabled={product.inStock === false} className="flex-grow bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-bold py-4 rounded-2xl shadow-lg shadow-indigo-200 transition-all flex items-center justify-center gap-2"><span className="material-icons">shopping_bag</span> Add to Cart</button>
              <button type="button" className="px-4 border border-gray-200 rounded-2xl hover:bg-gray-50 text-gray-400"><span className="material-icons">favorite_border</span></button>
            </div>
          </div>
        </div>
      </div>
