        ...
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between gap-8">
          ...
          <div className="flex-grow max-w-2xl relative">
            <input
              ...
              placeholder="Search for electronics, brands, or categories..."
              type="text"
            />
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex-1">
                      ...
                      <input
                        type="text"
                        defaultValue="$0"
                        className="w-full text-sm border border-slate-200 dark:border-slate-700 bg-transparent dark:bg-slate-800 rounded px-2 py-1"
                      />
                    </div>
                    <div className="flex-1">
              ...
              <div className="space-y-4">
                <div>
                  <button
                    ...
                    className="flex items-center justify-between w-full text-sm font-bold text-slate-500 uppercase tracking-wider mb-2"
                  >
                    Processor
                  ...
                  {processorOpen && (
                    <div className="space-y-2">
                      <label className="flex items-center gap-2 cursor-pointer">
                        ...
                        <span className="text-sm text-slate-600 dark:text-slate-400">Apple M3</span>
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" className="rounded border-slate-300 text-primary focus:ring-primary" />
                        <span className="text-sm text-slate-600 dark:text-slate-400">Intel Core i9</span>
                      </label>
                    </div>
                  )}
                </div>
                ...
                <div>
                  <button
                    ...
                    className="flex items-center justify-between w-full text-sm font-bold text-slate-500 uppercase tracking-wider"
                  >
                    Connectivity
        ...
        <div className="flex-grow">
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <select
                ...
                className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm px-4 py-2 focus:ring-primary min-w-[160px]"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {listingProducts.map((product) => (
              <div
                key={product.id}
                className="product-card bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden group flex flex-col"
              >
                <div className="relative aspect-square overflow-hidden bg-slate-50 dark:bg-slate-800/50">
                  ...
                  {product.dealOfTheDay && (
                    <div className="absolute top-3 left-3 bg-primary text-white text-[10px] font-bold uppercase px-2 py-1 rounded">
                      Deal of the Day
                    </div>
                  )}
                  <button
                    type="button"
                    className="absolute top-3 right-3 w-8 h-8 bg-white/80 dark:bg-slate-900/80 backdrop-blur rounded-full flex items-center justify-center text-slate-400 hover:text-red-500 transition-colors"
                  >
                ...
                <div className="p-5 flex flex-col flex-grow">
                  <div className="flex items-center gap-1 mb-2">
                    {[...Array(5)].map((_, i) => {
                      ...
                      return (
                        <span
                          key={i}
                          className={`material-icons text-sm ${filled ? 'text-amber-400' : 'text-slate-300'}`}
                        >
                          {i < full ? 'star' : half ? 'star_half' : 'star'}
                        </span>
                      );
                    })}
                  ...
                  <div className="mt-auto pt-4 flex items-end justify-between">
                    <div>
                      ...
                      {product.oldPrice && (
                        <span className="block text-sm text-slate-400 line-through">${product.oldPrice.toFixed(2)}</span>
                      )}
                    </div>
                  </div>
                  <Link
                    to={`/product/${product.productDetailId ?? product.id}`}
                    className="w-full mt-4 bg-primary hover:bg-primary/90 text-white font-medium py-2.5 rounded-lg flex items-center justify-center gap-2 transition-colors"
                  >
          ...
          <div className="mt-12 flex items-center justify-center gap-2">
            <button
              type="button"
              className="w-10 h-10 flex items-center justify-center rounded-lg border border-slate-200 dark:border-slate-800 text-slate-500 hover:border-primary hover:text-primary transition-colors"
            >
            ...
            {[1, 2, 3].map((n) => (
              <button
                ...
                className={`w-10 h-10 flex items-center justify-center rounded-lg font-medium transition-colors ${
                  n === 1 ? 'bg-primary text-white' : 'border border-slate-200 dark:border-slate-800 text-slate-500 hover:border-primary hover:text-primary'
                }`}
              >
                {n}
              </button>
        ...
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-8">
          <div className="space-y-4">
            ...
            <p className="text-sm text-slate-500 leading-relaxed">
              Your one-stop destination for premium electronics and smart home solutions.
            </p>
          </div>
          <div>
            ...
            <ul className="space-y-2 text-sm text-slate-500">
              ...
              <li><a className="hover:text-primary" href="#">Returns & Exchanges</a></li>
            </ul>
          </div>
          <div>
            <h4 className="font-semibold mb-4 text-slate-900 dark:text-white">Customer Support</h4>
            <ul className="space-y-2 text-sm text-slate-500">
              <li><a className="hover:text-primary" href="#">Help Center</a></li>
              <li><a className="hover:text-primary" href="#">Contact Us</a></li>
              <li><a className="hover:text-primary" href="#">Live Chat</a></li>
              <li><a className="hover:text-primary" href="#">Warranty Information</a></li>
            </ul>
          </div>
          <div>
            <h4 className="font-semibold mb-4 text-slate-900 dark:text-white">Newsletter</h4>
            <p className="text-sm text-slate-500 mb-4">Subscribe to get special offers and once-in-a-lifetime deals.</p>
